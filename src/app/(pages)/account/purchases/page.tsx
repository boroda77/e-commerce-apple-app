...
export default async function Purchases() {
  const { user } = await getMeUser({
    nullUserRedirect: `/login?error=${encodeURIComponent(
      'Ви повинні увійти в систему, щоб отримати доступ до свого облікового запису.',
    )}&redirect=${encodeURIComponent('/account')}`,
  })

  return (
    <div>
      <h5>Придбані продукти</h5>
      <div>
        {user?.purchases?.length || 0 > 0 ? (
                  ...
                  {typeof purchase === 'string' ? (
                    ...
                    <Link href={`/products/${purchase.slug}`} className={classes.item}>
                      <div className={classes.mediaWrapper}>
                        {!purchase.meta.image && (
                          <div className={classes.placeholder}>Немає зображення</div>
                        )}
                      ...
                      <div className={classes.itemDetails}>
                        ...
                        <p className={classes.purchasedDate}>{`Придбано: ${formatDateTime(
                          purchase.createdAt,
                        )}`}</p>
                      </div>
                    </Link>
          ...
          <div className={classes.noPurchases}>У вас немає покупок.</div>
        )}
      </div>
    </div>
  )
